...
AUDIO_RECOGNITION_URL = "http://127.0.0.1:5000/gpt"
...
def get_gpt_response(user_input, conversation_history):
    try:
        response = requests.post(AUDIO_RECOGNITION_URL, json={'user_input': user_input, 'conversation_history': conversation_history})
        ...
        conversation_history.append({"role": "assistant", "content": gpt_result['response']})
        ...
        gpt_response = {"resp_user": "No response from GPT", "context": ""}
...
def get_tts_audio(text):
    try:
        ...
        logging.error(f"Error in TTS request: {e}")
        return None

def main():
    global recording
    
    vosk_model = Model(r"C:\Users\user2\Desktop\avatar\vosk\vosk-model-kz-0.15")
    samplerate = 16000
    device = None
